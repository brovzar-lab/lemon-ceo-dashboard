...
function EmailRow({ email, onToggleImportant }: { email: Email; onToggleImportant: (id: string) => void }) {
  return (
    <Link
      to={`/inbox?id=${email.id}`}
      className={`flex items-start gap-3 px-4 py-3 hover:bg-slate-800/60 transition-colors border-b border-slate-800/60 ${
...
export function DashboardPage() {
  ...
  const importantEmails = emails.filter((e) => e.isImportant).slice(0, 8);
      ...
      <div className="flex-1 flex flex-col min-w-0 border-r border-slate-800">
        ...
        <div className="flex-1 overflow-y-auto">
          {importantEmails.map((email) => (
            <EmailRow key={email.id} email={email} onToggleImportant={handleToggleImportant} />
          ))}
        </div>
      </div>

      {/* Right: Today's Calendar + header */}
